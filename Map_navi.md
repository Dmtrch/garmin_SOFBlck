...
1. ~~**Permission `Communications` — убрать?**~~  
   ✅ **Закрыто**. Убран из `manifest_clear.xml`. Сборка всех 6 устройств — OK. `WatchUi.MapView` работает без сетевого разрешения (офлайн-карты на диске часов).
...
- [x] **Кнопки zoom**. Реализовано: START (onMenu) циклически переключает режим LAT→LON→ZOOM→LAT; в ZOOM-режиме UP/DOWN меняют масштаб 200/500/2000 м. Оверлей показывает три строки (жёлтая = активная). Сборка 6/6 OK.
